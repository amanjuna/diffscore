...
class Model():
    ...
    def add_loss_op(self, pred):
        ...
        with tf.variable_scope("loss", reuse=tf.AUTO_REUSE):
            # squared loss
            # recall that self.input is a (cell, weight, label) tuple
            loss = self.config.beta * tf.losses.mean_squared_error(self.input[2], pred, weights=self.input[1])
            self.squared = tf.losses.mean_squared_error(self.input[2], 
                                                        pred, weights=self.input[1],
